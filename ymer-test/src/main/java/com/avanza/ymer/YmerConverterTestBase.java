...
public abstract class YmerConverterTestBase {
	...
	@Test
	@Ignore("This should probably not be required? Is the description below even correct?")
	public void failForAutoGeneratedIdWithoutExplicitSpaceRouting() throws Exception {
		Object spaceObject = testCase.spaceObject;
		Optional<Method> autoGeneratedSpaceId = createStreamOfAllMethods(spaceObject)
				.filter(m -> m.isAnnotationPresent(SpaceId.class) && m.getAnnotation(SpaceId.class).autoGenerate())
				.findFirst();

		if (autoGeneratedSpaceId.isPresent()) {
			long numberOfAnnotatedMethods = createStreamOfAllMethods(spaceObject)
					.filter(m -> m.isAnnotationPresent(SpaceRouting.class))
					.count();

			assertEquals(
					"No @SpaceRouting annotation found for " + spaceObject.getClass().getName()
							+ ". All space objects with autogenerated SpaceId should have an explicit routing key. "
							+ "Changing number of partitions might lead to objects routed by autogenerated id ending up in undesired places",
					numberOfAnnotatedMethods, 1);
		}
	}
